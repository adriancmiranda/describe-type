...
test(`${colors.underline('describeType')} array`, (t) => {
	t.is(stringify([1/0, 'teste', 1, 2]), '[Infinity,teste,1,2]', 'should be a list of string');
});

test(`${colors.underline('describeType')} object`, (t) => {
	t.is(stringify({ a: 1/0, b: 'teste', c: 1 }), '{a:Infinity,b:teste,c:1}', 'should be a list of string');
});

test(`${colors.underline('describeType')} nested array`, (t) => {
	t.is(stringify([1/0, ['teste', 1], 2]), '[Infinity,[teste,1],2]', 'should be a list of string');
});

test(`${colors.underline('describeType')} nested object`, (t) => {
	t.is(stringify({
		a: 1/0,
		b: 'foo',
		c: 1,
